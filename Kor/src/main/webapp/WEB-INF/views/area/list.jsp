...
<nav class="navbar navbar-expand navbar-dark bg-primary"> 
     ...
     <span class="navbar-toggler-icon"></span> </button>
		...
		<div class="row">
			...
			<h1 style="text-align: center;">지역목록</h1>
			<form>	
			<div class="row">
				<div class="col-2">
						<select name="search" class="form-control form-control-sm">
						<option value="0">전체 검색</option>
						<option value="1" ${pager.search == 1 ? 'selected' : ''}>지역이름</option>					
					</select>
				</div>
			...
			<table class="table bg-white rounded border">
				...
				<c:forEach var="item" items="${list}">
					<tr>
						...
						<td>${item.areaMapx}</td>
						<td>${item.areaMapy}</td>
						<td><a href="delete?areaId=${item.areaId}">삭제</a> <a href="update?areaId=${item.areaId}">변경</a></td>
					</tr>
