...
<body>
				...
				<c:when test="${list.size() > 0}">
					<c:forEach var="item" items="${list}">
						<tr>
							...
							<td><a href="view/${item.noticeId}">${item.nSubject}</a></td>
							<td>${item.nWriter}</td>
							<td><fmt:formatDate value="${item.nDate}" pattern="yyyy-MM-dd" /></td>
							<td>${item.nviewCnt}</td>
	...
	<button type="button" class="btn btn-secondary" onclick=" location.href = '../admin/noticeadd';">글쓰기</button>
	</c:if>
